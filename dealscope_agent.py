...
# ---------------------------------------------------------------------------
# v1.1 — fixes: max_loops 2->5, relaxed disclaimer regex
# ---------------------------------------------------------------------------
...
STARTUP_FIELDS = [
    "company", "description", "product", "target_customer",
    "market", "team", "traction", "revenue_model",
    "pricing", "competitors", "differentiation",
    "go_to_market", "fundraising_stage", "known_risks",
]
...
def normalize_startup_profile(startup_description: str) -> dict[str, str]:
    ...
    profile = {f: "" for f in STARTUP_FIELDS}
    ...
    lines = [l.strip() for l in startup_description.splitlines() if l.strip()]
    for line in lines:
        if ":" not in line:
            continue
...
def assess_input_completeness(startup_profile: dict[str, str]) -> dict[str, Any]:
    ...
    present = [f for f in STARTUP_FIELDS if str(startup_profile.get(f, "")).strip()]
    missing = [f for f in STARTUP_FIELDS if f not in present]
    ...
    return CompletenessReport(
        ...
        present_fields=present, missing_fields=missing, warnings=warnings,
    ).to_dict()


def detect_sea_context(startup_profile: dict[str, str]) -> dict[str, Any]:
    """Detect whether SEA/Vietnam context should be applied."""
    text = " ".join(str(v) for v in startup_profile.values()).lower()
    locations = [
        "vietnam", "viet nam", "indonesia", "thailand",
        "philippines", "singapore", "malaysia", "southeast asia", "sea",
    ]
    matched = [loc for loc in locations if loc in text]
    return {
        ...
        "guidance": (
            ...
            if matched else "Do not force SEA context unless the user asks for it."
        ),
    }
...
def build_due_diligence_question_set(startup_profile: dict[str, str]) -> list[str]:
    ...
    for f in missing:
        if f in question_bank:
            questions.append(question_bank[f])
...
def validate_due_diligence_memo(memo: str) -> dict[str, Any]:
    ...
    lower = memo.lower()
    missing_sections = [s for s in REQUIRED_SECTIONS if s.lower() not in lower]
    warnings: list[str] = []

    if not re.search(r"financial|legal|investment advice|not.*advice|no.*advice", lower):
